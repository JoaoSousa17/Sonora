...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (accessCode.trim() !== "SonoraSecret") {
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Verifica o teu email para o novo código.",
      });
    } catch (err) {
  ...
  const handleGitHub = () => base44.auth.loginWithProvider("github", safeReturnTo());
  const handleMeta = () => base44.auth.loginWithProvider("facebook", safeReturnTo());
  ...
  if (showOtp) {
    return (
      <AuthLayout
        ...
        subtitle={`Enviámos um código para ${email}`}
      >
        {error && (
  ...
  return (
    <AuthLayout
      ...
      subtitle="Regista-te para começar"
      footer={
        <>
          Já tens conta?{" "}
          <Link
    ...
    >
      <div className="space-y-2.5 mb-6">
        ...
        <Button variant="outline" className="w-full h-12 text-sm font-medium" onClick={handleGitHub}>
          <GitHubIcon className="w-5 h-5 mr-2" />
          Continuar com GitHub
        </Button>
        <Button variant="outline" className="w-full h-12 text-sm font-medium" onClick={handleMeta}>
